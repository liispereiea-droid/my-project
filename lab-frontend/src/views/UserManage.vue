...
const loadData = async () => {
  try {
    const res = await request.get('http://localhost:8080/api/user/list', {
...
const submitForm = async () => {
  ...
  try {
    if (form.id) {
      ...
      const res = await request.put('http://localhost:8080/api/user/update', form)
      if (res.data.code === 200 ) {
      ...
      const res = await request.post('http://localhost:8080/api/user/add', form)
...
const handleDelete = async (id) => {
  try {
    const res = await request.delete(`http://localhost:8080/api/user/delete/${id}`)
    if (res.data.code === 200) {
