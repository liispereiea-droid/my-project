      ...
      <el-table :data="tableData" border stripe style="width: 100%" row-key="id">
        ...
        <el-table-column prop="categoryName" label="分类名称" width="200">
          <template #default="scope">
            <el-tag effect="dark">{{ scope.row.categoryName }}</el-tag>
          </template>
        </el-table-column>
...
const loadData = async () => {
  try {
    const [categoryRes, consumableRes] = await Promise.all([
      ...
      request.get('http://localhost:8080/api/consumable/list?labId=1')
    ])
    // 解包
    if (categoryRes.data.code === 200) tableData.value = categoryRes.data.data
    if (consumableRes.data.code === 200) allConsumables.value = consumableRes.data.data
...
const openDialog = (row = null) => {
  if (row) {
    form.id = row.id; form.categoryName = row.categoryName; form.remark = row.remark
  } else {
    form.id = null; form.categoryName = ''; form.remark = ''
  }
...
const handleDelete = async (id) => {
  try {
    await request.delete(`http://localhost:8080/api/category/delete/${id}`)
